...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    forkJoin({
      ...
      todas:    this.ordemService.listar(),
      abertas:  this.ordemService.listar('Aberta'),
      andamento:this.ordemService.listar('EmAndamento'),
    }).subscribe({
      next: ({ clientes, todas, abertas, andamento }) => {
        this.ordensAbertas = abertas;
        this.stats = [
          { label: 'Clientes',     value: clientes.length,  icon: '👥', color: '#1a56db' },
          { label: 'OS Abertas',   value: abertas.length,   icon: '📋', color: '#c27803' },
          { label: 'Em Andamento', value: andamento.length, icon: '🔧', color: '#6d28d9' },
          { label: 'Total de OS',  value: todas.length,     icon: '📁', color: '#057a55' },
        ];
        this.loading = false;
      },
